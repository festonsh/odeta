...
**Vercel demo login (no database required):** Use **demo@odetaa.com** with password **VercelDemo123!** (copy-paste to avoid typos). This user is not stored in the DB; it works without a database. If login fails, in Vercel → Project → Settings → Environment Variables remove `VERCEL_DEMO_PASSWORD` if it is set to empty.
